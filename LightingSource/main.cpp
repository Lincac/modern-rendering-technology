...
int main()
{
	...
	program.compile(R"(
		...
		void main()
		{
			...
			vs_out.LightPosViewSapce = vec3(view * model * vec4(LightPos, 1.0));
		...
		void main()
		{
			...
			if(LightMode == 0) // 平行光源
			{
				...
				float spec = pow(max(dot(normalVS, halfwayDir), 0.00001), 32);
			...
			{
				vec3 lightDir = normalize(fs_in.LightPosViewSapce - fs_in.FragPosViewSapce);
				float diff = max(dot(normalVS, lightDir), 0.00001);
				...
				float distance = length(fs_in.LightPosViewSapce - fs_in.FragPosViewSapce);
				float attenuation = 1.0 / (light_constant + light_linear * distance + light_quadratic * (distance * distance));

				vec3 correct = pow((diff + spec) * color, vec3(1.0/gamma));

				FragColor = vec4(correct * attenuation, 1.0);
			}
			else if(LightMode == 2)
			{
				vec3 lightDir = normalize(fs_in.LightPosViewSapce - fs_in.FragPosViewSapce);
				...
				float light_linear = 0.09;
				float light_quadratic = 0.032;
				float light_cutOff = 0.994056;
				float light_outerCutOff = 0.988362;

				float theta = dot(lightDir, normalize(fs_in.LightDirViewSapce));
	...
	std::cout << glm::cos(glm::radians(6.25)) << ' ' << glm::cos(glm::radians(8.75)) << std::endl;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		program.SetValue("LightPos", LightPos);
